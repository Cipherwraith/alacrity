...
-- Server Output
data ServerOut = ErrorMsg !(String) 
               | DataSaved !(String) 
               | ViewData !(FilePath) !(B.ByteString)
               | ViewRawData !(FilePath) !(B.ByteString)
...
data IndexSettings = IndexSettings {
    _assumeIndex     :: {-# UNPACK #-} !(Bool)
  , _indexAssumption :: {-# UNPACK #-} !(FilePath)
} deriving (Show)

data Msg = Msg {
    _command :: {-# UNPACK #-} !(String)
  , _password :: {-# UNPACK #-} !(Maybe String)
  , _filepath :: {-# UNPACK #-} !(FilePath)
  , _filedata :: {-# UNPACK #-} !(Maybe T.Text)
}
...
data PreppedFile = PreppedFile {
    _getText   :: {-# UNPACK #-} !(Maybe TL.Text)
  , _getBinary :: {-# UNPACK #-} !(Maybe BL.ByteString)
} 
